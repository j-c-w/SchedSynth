...
fn process(nesting_depth: i32, sequence: Pair<Rule>) -> SketchAST {
    ...
    match sequence.as_rule() {
        Rule::sequence_list => {
            ...
            let seq = process(nesting_depth, inner.next().unwrap());
            if inner.len() > 1 {
                ...
                let rest = process(nesting_depth, inner.next().unwrap()); // next seq list
        ...
        Rule::sequence => {
			...
			let stmt = process(new_nesting_depth, inner.next().unwrap());
			stmt
        },
		Rule::produce => {
			...
			// let _ = inner.next(); // whitespace
			let ident = match process(nesting_depth, inner.next().unwrap()) {
...
pub fn parse() {
  ...
  for x:
    ...
    let parsed = process(0, sequence.next().unwrap());
